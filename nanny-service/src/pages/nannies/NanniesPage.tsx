...
import type { NanniesPageState, NanniesPageActions, Nanny } from '../../lib/type/nannies';
...
const NanniesPage = () => {
  ...
  const fetchNannies = useCallback(async () => {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        // Firebase RTDB may return an array if keys are numeric indices
        const nanniesList = Array.isArray(data) 
          ? data.map((n, i) => ({ id: n.id || String(i), ...n }))
          : Object.keys(data).map(key => ({ id: key, ...(data[key] as Omit<Nanny, 'id'>) }));
        
        setState(s => ({ ...s, nannies: nanniesList as Nanny[], isLoading: false }));
      } else {
      ...
      try {
        ...
        setState(s => ({ ...s, nannies: data.nannies as Nanny[], isLoading: false }));
      } catch (localErr) {
        console.error('All data sources failed:', localErr);
        setState(s => ({ ...s, error: 'Failed to load nannies. Please check your connection.', isLoading: false }));
      }
    }
  }, []);
  ...
  const actions: NanniesPageActions = {
    fetchNannies,
    updateFilters: (filters) => setState(s => ({ ...s, filters: { ...s.filters, ...filters } })),
    loadMore: () => setState(s => ({ ...s, visibleCount: s.visibleCount + 3 })),
  ...
  const getFilteredNannies = () => {
    ...
    if (sort === 'price-low') filtered = filtered.filter(n => n.price_per_hour < 20); // Adjusted threshold based on real data
